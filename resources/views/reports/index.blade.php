    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                    <h3 class="font-semibold text-gray-900 mb-3">Expense by Category</h3>
                    @if ($categoryReport['expenses']->isEmpty())
                        <p class="text-gray-500 text-sm">No expenses in this period.</p>
                    @else
                        <div class="space-y-3">
                            @foreach ($categoryReport['expenses'] as $expense)
                                ...
                                <div>
                                    <div class="flex justify-between text-sm mb-1">
                                        <span class="text-gray-700">{{ $expense->category->name }}</span>
                                        <span class="text-gray-500">{{ number_format($expense->total, 2) }} ({{ $pct }}%)</span>
                                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                    <h3 class="font-semibold text-gray-900 mb-3">Income by Category</h3>
                    @if ($categoryReport['incomes']->isEmpty())
                        <p class="text-gray-500 text-sm">No income in this period.</p>
                    @else
                        <div class="space-y-3">
                            @foreach ($categoryReport['incomes'] as $income)
                                ...
                                <div>
                                    <div class="flex justify-between text-sm mb-1">
                                        <span class="text-gray-700">{{ $income->category->name }}</span>
                                        <span class="text-gray-500">{{ number_format($income->total, 2) }} ({{ $pct }}%)</span>
                                    </div>
                                    <div class="w-full bg-gray-200 rounded-full h-2">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                @if ($transactions->isEmpty())
                    <div class="p-5 text-gray-500 text-sm text-center">No transactions found.</div>
                @else
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach ($transactions as $t)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 text-sm text-gray-600">{{ $t->date->format('M d, Y') }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $t->description ?: '-' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-600">{{ $t->category->name }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-600">{{ $t->account->name }}</td>
