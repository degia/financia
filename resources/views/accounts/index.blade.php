<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('accounts.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                + New Account
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if ($accounts->isEmpty())
                <div class="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-200">
                    ...
                    <p class="text-gray-500 mb-6">Create your first account to start tracking your finances.</p>
                    <a href="{{ route('accounts.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                        + Create Account
                    </a>
                </div>
            @else
                            ...
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-xs text-gray-500 capitalize">{{ str_replace('_', ' ', $account->type) }}</span>
                                    </div>
                                </div>
