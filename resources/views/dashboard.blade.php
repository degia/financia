    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center gap-4">
            ...
            <div class="hidden sm:flex items-center gap-3 px-4 py-1.5 bg-gray-100 dark:bg-gray-800 rounded-lg">
                ...
                <span
                    class="text-sm font-bold text-gray-900 dark:text-white">{{ number_format($summary['realBalance'], 2) }}</span>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div x-data='dashboardCards(@json($cards), @json($defaultOrder))' class="mb-6">
                ...
                <div x-show="showSettings" x-cloak class="card p-4 mb-4">
                    ...
                    <button @click="resetOrder()"
                        class="mt-3 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white underline">Reset
                        to default</button>
                </div>
                ...
                <div class="card p-5">
                    ...
                    <div class="h-64">
                        <canvas id="monthlyChart" data-labels='@json($monthlyChart['labels'])'
                            data-income='@json($monthlyChart['income'])' data-expense='@json($monthlyChart['expense'])'>
                        </canvas>
                    </div>
                </div>
                <div class="card p-5">
                    <div class="flex flex-wrap items-start justify-between gap-3 mb-4">
                        <h3 class="font-semibold text-gray-900 dark:text-white">Expense by Category
                            ({{ Carbon\Carbon::createFromDate($year, $month, 1)->format('M Y') }})</h3>
                        @if ($categories->isNotEmpty())
                            <details class="relative text-xs">
                                <summary
                                    class="cursor-pointer text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white select-none">
                                    Filter Categories &blacktriangledown;
                                </summary>
                                <div
                                    class="absolute right-0 top-full mt-1 z-10 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-3 min-w-[180px] space-y-1.5">
                                    @foreach ($categories as $cat)
                                        <label class="flex items-center gap-2 cursor-pointer hover:opacity-80">
                                            ...
                                            <span class="w-2.5 h-2.5 rounded-full flex-shrink-0"
                                                style="background: {{ $cat->color ?? '#6B7280' }}"></span>
                    ...
                    <div class="h-64">
                        <canvas id="categoryChart" data-labels='@json($categoryBreakdown['labels'])'
                            data-data='@json($categoryBreakdown['data'])' data-colors='@json($categoryBreakdown['colors'])'>
                        </canvas>
                    </div>
                    @if (empty($categoryBreakdown['labels']))
                        <p class="text-center text-gray-400 dark:text-gray-500 text-sm mt-3">No data for the selected
                            filters.</p>
                    @endif
                </div>
            </div>
            ...
            <div class="card p-5 mb-6">
                <div class="flex flex-wrap items-center justify-between gap-2 mb-4">
                    <h3 class="font-semibold text-gray-900 dark:text-white">Daily Income vs Expense
                        ({{ Carbon\Carbon::create()->month($month)->format('F Y') }})</h3>
                    <div class="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                        <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-sm bg-green-500"></span>
                            Income</span>
                        <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-sm bg-red-500"></span>
                            Expense</span>
                    </div>
                </div>
                <div class="h-72">
                    <canvas id="dailyChart" data-labels='@json($dailyChart['labels'])'
                        data-income='@json($dailyChart['incomeData'])' data-expense='@json($dailyChart['expenseData'])'>
                    </canvas>
                </div>
            </div>
                        ...
                        @foreach ($budgetProgress as $bp)
                            <div>
                                <div class="flex justify-between text-sm mb-1">
                                    ...
                                    <span
                                        class="text-gray-500 dark:text-gray-400">{{ number_format($bp['spent'], 0) }}
                                        / {{ number_format($bp['budgeted'], 0) }} ({{ $bp['percentage'] }}%)</span>
                                </div>
                                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                                    <div class="h-2.5 rounded-full transition-all duration-300 {{ $bp['percentage'] > 100 ? 'bg-red-500' : ($bp['percentage'] > 80 ? 'bg-orange-500' : ($bp['percentage'] > 50 ? 'bg-yellow-500' : 'bg-green-500')) }}"
                                        style="width: {{ min(100, $bp['percentage']) }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="card p-5">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="{{ route('transactions.index') }}"
                        class="text-sm text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white">View
                        All</a>
                </div>
                            ...
                            <div class="flex justify-between items-center py-3">
                                <div>
                                    <p class="text-sm font-medium text-gray-900 dark:text-white">
                                        ...
                                        @if ($t->is_recurring)
                                            <span
                                                class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 ml-1">
